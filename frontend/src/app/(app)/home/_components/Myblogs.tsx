...
interface MyBlogsProps {
  // initialData: GetMyBlogsResponse;
}
...
const MyBlogs: FC<MyBlogsProps> = ({}) => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const initialData = await blogService.getMyBlogs();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const data = await blogService.getMyBlogs(pagination.currentPage);
